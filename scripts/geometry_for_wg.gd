...
func create_chunc_at(pos):
	...
	for x in [-1,0,1]:
		for y in [-1,0,1]:
			var chunk_pos = Vector2(x, y)*16 + pos
			if not chunk_pos in chunk_dict:
				chunk_dict[chunk_pos] = chunkClass.new(chunk_pos,self)
			if not chunk_dict[chunk_pos].isgenerated:
				chunk_dict[chunk_pos].generate()
				nothing_done = false
...
func _process(delta):
	...
	if ringlist.size() > 0 and processing_chunks.size() == 0:
		while create_chunc_at(ringlist[0]): #returns true if nothing was done
			ringlist.remove(0)
		if ringlist.size() > 0:
			ringlist.remove(0)
...
#			print("existieret",v_pos in chunk_dict[correctChunkPos].ChunkVoxel)
		return v_pos in chunk_dict[correctChunkPos].ChunkVoxel
	return false
